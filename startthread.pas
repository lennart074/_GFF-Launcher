...
Type
  TStartThread = Class(TThread)
    ...
    Procedure GetInformation_type_MC_Forge();
    Procedure StartMC_type_MC_Forge();
    Procedure GetInformation_type_ModPack_Technic();
    Procedure StartMC_type_ModPack_Technic();
...
Begin
  Try
    ...
    Begin
      GetInformation_type_ModPack_Technic();
    End;}
...
//##############################################################################
//##############################################################################
Procedure TStartThread.GetInformation_type_MC_Forge();
Var
...
Begin
  ...
  If (profileSettings.ReadBool('Profile', 'forge-ready', False) = False) Or
  ...
  Try
    ...
    currStr := StringReplace(currStr, ':true', ':"True" ', [rfReplaceAll, rfIgnoreCase]);

    CurrentAction := 'Exctracting Lib. info...';
    Synchronize(@ShowStatus);
  ...
  Except
    ...
    Begin
      ...
      Else
      Begin
        ErrorMsg := '@Major_n_LibsExtract' + LineEnding + E.ClassName +
          LineEnding + E.Message + LineEnding + currStr;
      End;
      Exit;
    End;
  End;
...
Procedure TStartThread.GetInformation_type_ModPack_Technic();
Var
  SubIndexURL, AssetIndexFile, AssetIndexURL, LibStr, currStr, tempURL, NNLibUrl: String;
  P: TJSONParser;
  D: TJSONData;
...
Begin
  ...
  //Download SubIndex
  If (Not FileExists(ProfilePath + '/bin/modpack.jar')) Then
  Begin
    SubIndexURL := webSettings.ReadString('DownloadLocation', 'SubIndex',
      'http://s3.amazonaws.com/Minecraft.Download/versions/%version%/%version%.json');
    SubIndexURL := (StringReplace(SubIndexURL, '%version%', version, [rfReplaceAll]));
  ...
  Begin
    ...
    Unzipper.FileName := ProfilePath + '/bin/modpack.jar';
    Unzipper.OutputPath := 'GFFLauncher/temp';
    Unzipper.UnZipAllFiles;
    FreeAndNil(Unzipper);
    DeleteFile(ProfilePath + '/versions/' + version + '/' + version + '.json');
    CopyFile('GFFLauncher/temp/version.json', ProfilePath + '/versions/' +
...
Procedure TStartThread.StartMC_type_MC_Forge();
Var
  content, tempList: TStringList;
  P: TJSONParser;
  D: TJSONData;
  launchArgs, LibStr, startAct, oldDir: String;
  i, argLoc: Integer;
Begin
  Try
    ...
    launchArgs := StringReplace(launchArgs, '${auth_session}',
      UsrObj.accessToken, [rfReplaceAll]);
    ...
    Begin
      ...
      mcProc.Options := [poUsePipes, poStderrToOutPut, poNoConsole];
      Form_consoles.Show;
    End;
...
Procedure TStartThread.StartMC_type_ModPack_Technic();
Var
  content, tempList: TStringList;
  P: TJSONParser;
  D: TJSONData;
...
Begin
  ...
  Try
    ...
    LibStr := '"' + mcProc.CurrentDirectory + LibList[0] + '"';
    //LibList.Add('/versions/' + version + '/' + version + '.jar');
    If (FileExists(ProfilePath + '/bin/modpack.jar')) Then
    Begin
      LibList.add('/bin/modpack.jar');
    End;
    ...
    Begin
      startAct := 'Set options';
      mcProc.ShowWindow := swoShowDefault;
      mcProc.Options := [poUsePipes, poStderrToOutPut, poNoConsole];
      Form_consoles.Show;
    End;
    startAct := 'Adding Parameters';
    ...
    While ((mcProc.Running) And (Terminated = False) And
        ((MainSettings.showMCConsole = True) Or
        (profileSettings.ReadBool('Minecraft', 'showCons', False) = True))) Do
    Begin
      logLine := StreamToString(mcProc.Output);
      //If Length(LogLine) > 1 Then
